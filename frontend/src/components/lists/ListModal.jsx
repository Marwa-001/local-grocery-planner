...
export default function ListModal({ listId, onClose }) {
  const { lists, products, deleteListItem, togglePurchaseStatus, updateListItemQuantity, addProductToList, favourites } = useShoppingStore();
  ...
  const favouriteProducts = products.filter((p) => 
    favourites.some((f) => f.productId === p.id && f.userId === "local-user")
  );
